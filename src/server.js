...
// Streams every file so the PC window stays responsive on big folders, and
// skips (and counts) anything it can't read instead of giving up.
async function rebuildIndex(folder) {
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      if (entry.name.startsWith(MANIFEST_NAME) || entry.name === TMP_DIR_NAME) continue;
  ...
  await walk(folder);
  manifestEntry(folder).manifest = manifest;
  writeManifestNow(folder);
  return { count, skipped };
}

module.exports = { startServer, stopServer, rebuildIndex, cleanupTemp, flushManifests };
